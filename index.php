...
<html lang="en">
    ...
    <body id=body onload='preset("test.jpg")'>
        <div id=sidebar>
            ...
            <div id=imgselect title="You can drag and drop images here">
                <form>
                    
					<table>
                    <tr><td>Canvas:</td><td>W:<input id=pcw type=number min=10 value=800 oninput='changeSize()'>px </td>
                    <td>H:<input id=pch type=number min=10 value=600 oninput='changeSize()'>px</td></tr>
<!--                     <tr><td>Output:</td><td>W:<input id=pcwm type=number min=10 value=40 oninput='changeSize()'>mm</td>
                    <td>H:<input id=pchm type=number min=10 value=30 oninput='changeSize()'>mm</td></tr>
-->					</table>
                </form>
                <canvas></canvas><br>
            ...
            <div id=colormode>
                Color mode:<br> 
                    <input type="radio" id="cmy" name="colorMode" value="CMY" onclick='update()' checked title="Cyan, Magenta, Yellow as for printers">
                    <label for="cmy" title="Cyan, Magenta, Yellow as for printers"> CMY</label> 
                    <input type="checkbox" id="cmyk" name="colorModeCMYK" value="CMY" onclick='update()' checked title=" and Black">
                    <label for="cmyk" title=" and Black"> ...and K</label> <br>

                    <input type="radio" id="rgb" name="colorMode" value="RGB" onclick='update()' title="Red, Green, Blue as for monitors">
                    <label for="rgb" title="Red, Green, Blue as for monitors"> RGB</label>           
            </div>
    ...
    canvas_y=document.createElement("canvas")
    canvas_k=document.createElement("canvas")
...
//			pchm.value = pcwm.value * ch/cw;
        ...
        img.src = source;//'test.jpg';   //'https://grbl-plotter.de/plotterfun/grbl.jpg';
    }
...
/** Drag & Drop  https://wiki.selfhtml.org/wiki/JavaScript/File_Upload#Auswahl_mit_Drag_und_Drop ****/
	function dateiauswahl(evt) {
		...
		img.src = URL.createObjectURL(gewaehlteDateien[0]);
	}
  ...
  // Initialisiere Drag&Drop EventListener
	var dropZone = document.getElementById('imgselect');
...
/*********************/	
    ...
    function snapshot(){
        ...
        imgData = ctx.getImageData(0, 0, video.videoWidth, video.videoHeight);
        
        [canvas_c.width, canvas_c.height] = [video.videoWidth, video.videoHeight];
        [canvas_m.width, canvas_m.height] = [video.videoWidth, video.videoHeight];
        [canvas_y.width, canvas_y.height] = [video.videoWidth, video.videoHeight];
        [canvas_k.width, canvas_k.height] = [video.videoWidth, video.videoHeight];
    ...
    function process(){
        ...
        applyFilter(1,ctx_m);
        applyFilter(2,ctx_y);
        if (!modeRGB) applyFilter(3,ctx_k);
		
        resetSVG();
        loadWorker(window.algo);
        color_selector = 'c';
        sendToWorker([ window.config, ctx_c.getImageData(0,0,config.width,config.height) ]);
    }
    ...
    function loadWorker(src){
        ...
        myWorker.onmessage = function(msg) {
            ...
            // vector data result
                ...
                if (color_selector == 'c')
                {   mainpath_c.setAttributeNS(null, "d", data);
                    color_selector = 'm';
                    setTimeout(sendToWorker([ window.config, ctx_m.getImageData(0,0,config.width,config.height) ]),10);
                }
                else if (color_selector == 'm')
                {   mainpath_m.setAttributeNS(null, "d", data);
                    color_selector = 'y';
                    sendToWorker([ window.config, ctx_y.getImageData(0,0,config.width,config.height) ]);
                }
                else if (color_selector == 'y')
                {   mainpath_y.setAttributeNS(null, "d", data);
                    if (!modeRGB) {
                        color_selector = 'k';
                        sendToWorker([ window.config, ctx_k.getImageData(0,0,config.width,config.height) ]);
                    }
//					setTimeout(createlink, 500); // doesn't work
                }
                else if (color_selector == 'k')
                {   mainpath_k.setAttributeNS(null, "d", data);}
            } 
        }
    }
...
//        svg.setAttribute("width",pcwm.value+"mm");
//        svg.setAttribute("height",pchm.value+"mm");
        svg.setAttribute("width",config.width);
        svg.setAttribute("height",config.height);
//        svg.setAttribute("viewBox", '0 0 '+pcwm.value+'mm '+pchm.value+'mm');
        svg.setAttribute("viewBox", `0 0 ${config.width} ${config.height}`);
        svg.style.background=config.Inverted?"black":"white";
//        window.mainpath=document.createElementNS(svgNS, "path");
        window.mainpath_c=document.createElementNS(svgNS, "path");  // cyan
        window.mainpath_m=document.createElementNS(svgNS, "path");  // magenta
        window.mainpath_y=document.createElementNS(svgNS, "path");  // yellow
        window.mainpath_k=document.createElementNS(svgNS, "path");  // yellow
        mainpath_k.setAttributeNS(null, "id", "black");
        mainpath_k.setAttributeNS(null, "style", "stroke:" + (config.Inverted?"black":"black")+";stroke-width:0.2mm;fill:none;");
		
        if (modeRGB) {
            mainpath_c.setAttributeNS(null, "id", "red");
            mainpath_m.setAttributeNS(null, "id", "green");
            mainpath_y.setAttributeNS(null, "id", "blue");
            mainpath_c.setAttributeNS(null, "style", "stroke:" + (config.Inverted?"cyan":"red")+";stroke-width:0.2mm;fill:none;");  
            mainpath_m.setAttributeNS(null, "style", "stroke:" + (config.Inverted?"magenta":"green")+";stroke-width:0.2mm;fill:none;");
            mainpath_y.setAttributeNS(null, "style", "stroke:" + (config.Inverted?"yellow":"blue")+";stroke-width:0.2mm;fill:none;");
        } else {
            mainpath_c.setAttributeNS(null, "id", "cyan");
            mainpath_m.setAttributeNS(null, "id", "magenta");
            mainpath_y.setAttributeNS(null, "id", "yellow");
            mainpath_c.setAttributeNS(null, "style", "stroke:" + (config.Inverted?"red":"cyan")+";stroke-width:0.2mm;fill:none;");
            mainpath_m.setAttributeNS(null, "style", "stroke:" + (config.Inverted?"green":"magenta")+";stroke-width:0.2mm;fill:none;");
            mainpath_y.setAttributeNS(null, "style", "stroke:" + (config.Inverted?"blue":"yellow")+";stroke-width:0.2mm;fill:none;");
        }
        mainpath_m.setAttributeNS(null, "transform", "translate(1,1)");
        mainpath_y.setAttributeNS(null, "transform", "translate(2,2)");
        mainpath_k.setAttributeNS(null, "transform", "translate(3,3)");
        svg.appendChild(mainpath_c);
        svg.appendChild(mainpath_m);
        svg.appendChild(mainpath_y);
        if (!modeRGB && modeK) svg.appendChild(mainpath_k);
    }

	function createlink(){	// https://stackoverflow.com/questions/12255444/copy-svg-images-from-browser-to-clipboard
        const svgString = (new XMLSerializer()).serializeToString(svg);
		link.href="data:image/svg;base64,"+btoa(svgString);
	}
    function download(){
